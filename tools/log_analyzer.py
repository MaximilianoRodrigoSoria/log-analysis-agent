import re
from typing import List, Dict, Optional

HEADER_RE = re.compile(
    r"^(?P<ts>\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\s+"
    r"(?P<level>ERROR|WARN|INFO)\s+\[(?P<thread>[^\]]+)\]\s+"
    r"(?P<logger>[\w\.\$]+)\s+-\s+(?P<message>.*)$"
)

EXC_RE = re.compile(r"^(?P<exc>[a-zA-Z0-9\._$]+Exception|Error)(?::\s*(?P<excmsg>.*))?$")
FRAME_RE = re.compile(r"^\s*at\s+(?P<where>[\w\.\$]+)\((?P<file>[^:]+):(?P<line>\d+)\)\s*$")

def analyze_log(log_text: str) -> Dict:
    """
    Parse de logs multilinea:
    - Agrupa entradas por header timestamp+level+logger
    - Si es ERROR/WARN, captura exception (si existe) y top frame
    - Devuelve una estructura lista para que el LLM redacte un reporte
    """
    lines = log_text.splitlines()
    events: List[Dict] = []

    current: Optional[Dict] = None
    stack: List[str] = []

    def flush():
        nonlocal current, stack
        if not current:
            return

        # Detectar excepción y top frame dentro del bloque acumulado
        exc = None
        excmsg = None
        top_frame = None

        for ln in stack:
            m = EXC_RE.match(ln.strip())
            if m and not exc:
                exc = m.group("exc")
                excmsg = (m.group("excmsg") or "").strip() or None

            fm = FRAME_RE.match(ln)
            if fm and not top_frame:
                top_frame = {
                    "where": fm.group("where"),
                    "file": fm.group("file"),
                    "line": int(fm.group("line")),
                }

        current["exception"] = exc
        current["exception_message"] = excmsg
        current["top_frame"] = top_frame
        current["raw_block"] = "\n".join(stack).strip() or None

        events.append(current)
        current = None
        stack = []

    for ln in lines:
        h = HEADER_RE.match(ln)
        if h:
            flush()
            current = h.groupdict()
            stack = []
        else:
            # Línea suelta asociada al último header (stacktrace u otra info)
            if current is not None:
                if ln.strip() != "":
                    stack.append(ln)

    flush()

    # Agregados útiles
    errors = [e for e in events if e["level"] == "ERROR"]
    warns = [e for e in events if e["level"] == "WARN"]

    # Agrupar errores por excepción + top frame (si hay)
    groups: Dict[str, Dict] = {}
    for e in errors:
        key = f'{e.get("exception")}|{(e.get("top_frame") or {}).get("where")}|{(e.get("top_frame") or {}).get("line")}'
        g = groups.setdefault(key, {
            "count": 0,
            "exception": e.get("exception"),
            "top_frame": e.get("top_frame"),
            "logger": e.get("logger"),
            "samples": [],
            "first_ts": e.get("ts"),
            "last_ts": e.get("ts"),
        })
        g["count"] += 1
        g["last_ts"] = e.get("ts")
        if len(g["samples"]) < 2:
            g["samples"].append({
                "ts": e.get("ts"),
                "message": e.get("message"),
                "exception_message": e.get("exception_message"),
            })

    return {
        "summary": {
            "total_events": len(events),
            "total_errors": len(errors),
            "total_warnings": len(warns),
        },
        "error_groups": list(groups.values()),
        "warnings": warns[:10],  # límite para no inflar contexto
        "events": events[:50],   # límite razonable para el prompt
    }